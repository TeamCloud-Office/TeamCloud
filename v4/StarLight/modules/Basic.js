...
(function () {
    ...
    */
    ...
    function random(per) {
        if ((Math.floor(Math.random() * 100) + 10) <= per) {
            return true;
        } else {
            return false;
        }
     ...
     * @param {Boolean} r 전송 메시지
     * @returns
     */
    function post(s, m, an, r) {
        let data = JSON.parse(FS.read(SetPath));
        let snd = data["snd"];

        let stc = {
            sender: User.get(s)["name"] + "(" + User.get(s)["id"] + ")",
            time: getDate.today("/"),
            content: m,
            admin: an
        };
    ...
    function Coin(s, e, c, p, an) {
        ...
        try {
            ...
            Log.e(msg.error(e.name, e.fileName, e.message, e.lineNumber)) //오류 로그 생성
        }
    }
    ...
    function Nickname(s, e, n, pm, p, an) {
        try {
            ...
            if (pm == true) { //호칭 지급
                ...
                (Udata["nickname"]).unshift(n); //유저 데이터에서 s의 nickname 리스트에 n 값 추가한 값을 새로 삽입
                User.put(Udata["id"], Udata) //유저 데이터에 변경된 값 저장
                ...
                Udata["nickname"] = (Udata["nickname"]).filter(item => item != n); //유저 데이터에서 s의 nickname 리스트에 n 값 제거한 값을 새로 삽입
            ...
            Log.e(msg.error(e.name, e.fileName, e.message, e.lineNumber))
        }
    }
    ...
    */
    ...
    /**
     * 
     * @param {Number} type 0: TeamCloud | 3: Melon
     * @param {String} id 
     * @param {Object} data 
     * @param {String} room 
     */

    function KakaoLink(type, id, data, room) {
        try {
            let {
                KakaoApiService,
                KakaoShareClient
            ...
            let service = KakaoApiService.createService();
            let client = KakaoShareClient.createClient();

            let cookies = service.login({
                signInWithKakaoTalk: true
            }).awaitResult();

            if (type == 0) {
                client.init('5a3e53c51bd006035a5a8f889f85ff0d', 'https://www.team-cloud.kro.kr', cookies);

                Log.d("kakaolink", "전송 준비")
                client.sendLink(room, {
                    templateId: id,
                    templateArgs: data,
                }, 'custom').awaitResult();
                Log.d("kakaolink", "전송 완료")
                return "Success"
            }

            if (type == 3) {
                client.init('4d545a185d172754667d621049004aa1', 'https://m2.melon.com', cookies);
     ...
     * @param {Array} labels 
     * @param {String} lable 
     * @param {Array} datasets label: String, data: Array
     * @returns 
     */
    function graph(type, labels, datasets, title) {
        try {
    ...
    function image(image, id) {
        return JSON.parse(org.jsoup.Jsoup.connect("https://api.cloudinary.com/v1_1/teamcloud/image/upload")
            .data('file', 'data:image/png;base64,' + image)
            .data('upload_preset', 'profile')
            .data('public_id', id)
            .ignoreContentType(true)
            .post()
            .text()
    ...
    function githubAPI(owner, repo, path) {
        ...
        let res = org.jsoup.Jsoup.connect(url)
            ...
            .execute()
            .body();
        return java.lang.String(java.util.Base64.getDecoder().decode(JSON.parse(res)["content"].replace(/\n/g, "")), "UTF-8");
        } catch(e) {
            return e;
        }
    }
