...
(function () {
    ...
    toFixed = (n) => Math.floor(1e4 * n) / 1e4
    ...
    /**
     * 
     * @param {function|boolean} call 
     * @param {number} s start_delay
     * @param {number} re repeat_delay
     * @param {Objects} timer 
     */
    ...
    function get_delay(start_time, end_time) {
        ...
        if ((start_time < now_time && end_time > now_time) || start_time == now_time) { // 이미 실행중
            delay = 0
        } else if (now_time >= end_time) { // 오늘 실행 남은거 없을때
            time.setHours(24, 0, 0, 0)
            delay += time.getTime() + (1000 * 60 * 60 * start_time)
        } else if (start_time > now_time) { // 오늘 시작 안했을때
    ...
    * type1 이 0 일떄 구매 / 1일떄 판매 ///// type2이 0일떄 즉시처리 / 1일떄 예약처리
