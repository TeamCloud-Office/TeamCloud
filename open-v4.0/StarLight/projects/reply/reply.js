...
function onMessage(event) {
    ...
    let Set = JSON.parse(FS.read(SP)) //Setting Data


    let Udata = User.get(event.sender.name) //User Data


    if (event.message.startsWith(prefix + "도움말")) {
        event.room.send(msg.noti + 'https://www.team-cloud.kro.kr/blog/manual'); // 도움말 url 출력
    }
    ...
    if (event.message.startsWith(prefix)) {
        ...
        switch (event.message.replace(prefix, "")) {
            ...
            case "":
                ...
                event.room.send(Like(event.sender.name, true, 70))
                break;
    ...
    if (event.message.startsWith(prefix + "가위바위보 ")) {
        ...
        switch (cut[2]) {
            case '바위':
                ...
                event.room.send(Like(event.sender.name, true, 40)) // 호감도
                break;
            case '가위':
