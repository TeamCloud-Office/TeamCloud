...
(function () {
    ...
    let company = {
        // "이름" : [가격:Number,  판매가능유무:Boolean[unfinished],  분류:Number[0:IT, 1:보건(건강), 2:환경, 3:엔터테이먼트],  기업정보:String[], 가격변동:Number[] ]
        // index 3,4 는 자동으로 채워집니다

        "TC반도체": [74000, true, 0, "", 0, []],
        //TeamCloud 반도체 - 반도체 주식회사(IT)
        "For_health": [50000, true, 1, "", 0, []],
    ...
    function f_delay(call, s, re) {

        if (typeof call !== "boolean" && ((typeof s !== "number" || s < 0) || (typeof re !== "number" || re < 0))) return Log.d(s + "/" + re)

        if (typeof call == "boolean") return timer.cancel()

        let timerTask = java.util.TimerTask({
            run: function () {
                call()
            }
        })
    ...
    function get_delay(s_t, e_t) {
        ...
        if ((time.getHours() > s_t && time.getHours() < e_t) || time.getHours() == s_t) {
            delay = undefined
            Log.d("타입1")
        } else if (time.getHours() < s_t) {
            time.setHours(0, 0, 0, 0) //오늘중으로
            delay += time.getTime() + (1000 * 60 * 60 * s_t)
            Log.d("타입2")
        } else if (time.getHours() > e_t || time.getHours() == e_t) {
            time.setHours(24, 0, 0, 0); //다음날
            delay += time.getTime() + (1000 * 60 * 60 * s_t)
            Log.d("타입3")
        }
     ...
     * @param {*} min
     */
    function get_remain(hour, min) {

        let time = new Date()
        delay = -(time.getTime())

        if (time.getHours() < hour || (time.getHours() == hour && time.getMinutes() < min)) {
            time.setHours(24, 0, 0, 0)
            delay += time.getTime() + (1000 * 60 * 60 * hour) + (1000 * 60 * min)
        } else if (time.getHours() > hour || (time.getHours() == hour && time.getMinutes() > min)) {
            time.setHours(0, 0, 0, 0)
            delay += time.getTime() + (1000 * 60 * 60 * hour) + (1000 * 60 * min)
        } else if (time.getHours() == hour && time.getMinutes() == min) {
            delay = 0
        }
    ...
    function price (delay) {
        ...
        if (result == new Date().toLocaleDateString()) return price(get_delay()) //이미 오늘 1회이상 실행되었을떄 (점검을 진행했을시 무시하고 작동)
        f_delay(() => {
            if (!Api.isOn(ProjectManager.project.info.name)) {
            ...
            if (open[1] == new Date().getHours() && new Date().getMinutes() != 0) return price(get_delay()) //장마감
            ...
            for (let i = 0; i <= Object.keys(company).length - 1; i++) {
                ...
                if (company[Object.keys(company)[i]][4] == 2) {
                    company[Object.keys(company)[i]][0] = company[Object.keys(company)[i]][0] + (company[Object.keys(company)[i]][0] * Math.random() * 0.01) - (company[Object.keys(company)[i]][0] * Math.random() * 0.01)
                }
                if (company[Object.keys(company)[i]][4] == 0) {
                    if (Math.random() > 0.4) { //60%
                        company[Object.keys(company)[i]][0] = company[Object.keys(company)[i]][0] + (company[Object.keys(company)[i]][0] * Math.random() * 0.01);
